...
def release_stale_claims(
    ...
    *extended* (with a ``claim_extended`` event) instead of being
    ...
    stale = conn.execute(
        ...
        "       t.last_heartbeat_at, r.worker_start_time "
        "FROM tasks t "
    ...
    for row in stale:
        ...
        worker_state = _worker_pid_state(
            row["worker_pid"], expected_start_time,
        )
...
def reclaim_task(
    ...
    *,
    ...
    """
    row = conn.execute(
        "SELECT t.status, t.claim_lock, t.worker_pid, r.worker_start_time "
        "FROM tasks t "
        "LEFT JOIN task_runs r "
        "  ON r.id = t.current_run_id AND r.worker_pid = t.worker_pid "
        "WHERE t.id = ?",
...
_recent_worker_exit_start_times: dict[int, int] = {}
_recent_worker_returncode_start_times: dict[int, int] = {}
_owned_worker_pids: set[int] = set()
_owned_worker_processes: dict[int, Any] = {}
_owned_worker_process_groups: dict[int, int] = {}
_owned_worker_start_times: dict[int, int] = {}
...
def _record_worker_exit(pid: int, raw_status: int) -> None:
    ...
    with _owned_worker_pids_lock:
        worker_start_time = _owned_worker_start_times.get(pid)
    _recent_worker_exits[pid] = (int(raw_status), now)
    _recent_worker_returncodes.pop(pid, None)
    _recent_worker_returncode_start_times.pop(pid, None)
    if worker_start_time is not None:
        _recent_worker_exit_start_times[pid] = worker_start_time
    else:
        _recent_worker_exit_start_times.pop(pid, None)
    if len(_recent_worker_exits) > _RECENT_WORKER_EXITS_MAX // 2:
        ...
        for expired_pid in expired:
            ...
            _recent_worker_exit_start_times.pop(expired_pid, None)
...
def _record_worker_returncode(pid: int, returncode: int) -> None:
    ...
    _recent_worker_exit_start_times.pop(pid, None)
    if worker_start_time is not None:
        _recent_worker_returncode_start_times[pid] = worker_start_time
    else:
        _recent_worker_returncode_start_times.pop(pid, None)
    if len(_recent_worker_returncodes) > _RECENT_WORKER_EXITS_MAX // 2:
        ...
        for expired_pid in expired:
            ...
            _recent_worker_returncode_start_times.pop(expired_pid, None)
    ...
    * ``"unknown"`` — pid was not in the reap registry (either reaped by
    ...
    if returncode_entry is not None:
        if (
            ...
            and _recent_worker_returncode_start_times.get(int(pid))
    ...
    if (
        expected_start_time is not None
        and _recent_worker_exit_start_times.get(int(pid))
        != int(expected_start_time)
    ):
        return ("unknown", None)
    raw, _ = entry
...
def _discard_owned_worker(pid: int) -> None:
    with _owned_worker_pids_lock:
        ...
        _owned_worker_start_times.pop(int(pid), None)
...
def _terminate_owned_worker_descendants(pid: int) -> bool:
    ...
    with _owned_worker_pids_lock:
        ...
        worker_start_time = _owned_worker_start_times.get(int(pid))
    if process_group is None or not _process_group_alive(process_group):
        return True
    termination = _terminate_reclaimed_worker(
        int(pid),
        _claimer_id(),
        expected_start_time=worker_start_time,
    )
...
def _worker_pid_state(pid: Optional[int], expected_start_time: Optional[int]) -> str:
    ...
    if expected_start_time is None:
        return "alive"
    try:
        current_start_time = get_process_start_time(int(pid))
    except Exception:
        current_start_time = None
    if current_start_time is None:
        return "unavailable"
    if int(current_start_time) != int(expected_start_time):
        return "reused"
    return "alive"


def _worker_pid_alive(
    pid: Optional[int], expected_start_time: Optional[int],
) -> bool:
    return _worker_pid_state(pid, expected_start_time) in {"alive", "unavailable"}
...
def _terminate_reclaimed_worker(
    ...
    *,
    ...
    worker_state = _worker_pid_state(pid, expected_start_time)
    ...
    if worker_state == "dead" and (
        ...
        info["terminated"] = True
        return info
    if worker_state == "unavailable" and expected_start_time is not None:
        # A durable identity exists but cannot currently be verified.  Hold
        # the claim rather than signalling an unverified process or spawning
...
def enforce_max_runtime(
    ...
    *,
    ...
    rows = conn.execute(
        ...
        "       t.max_runtime_seconds, t.claim_lock, r.worker_start_time "
        "FROM tasks t "
...
def detect_crashed_workers(conn: sqlite3.Connection) -> list[str]:
    ...
    with write_txn(conn):
        ...
        for row in rows:
            ...
            pid = int(row["worker_pid"])
            kind, code = _classify_worker_exit(pid, expected_start_time)
...
def _register_owned_worker_pid(pid: int) -> Optional[int]:
    pid = int(pid)
    with _owned_worker_pids_lock:
        cached_start_time = _owned_worker_start_times.get(pid)
    worker_start_time = cached_start_time
    if worker_start_time is None:
        try:
            worker_start_time = get_process_start_time(pid)
        except Exception:
    ...
    with _owned_worker_pids_lock:
        ...
        if worker_start_time is not None:
            _owned_worker_start_times[pid] = int(worker_start_time)
        if process_group is not None:
            _owned_worker_process_groups[pid] = process_group
    return int(worker_start_time) if worker_start_time is not None else None
...
def _terminate_spawned_worker_before_requeue(
    ...
    worker_start_time = _register_owned_worker_pid(pid)
    termination = _terminate_reclaimed_worker(
        pid,
        claim_lock,
        expected_start_time=worker_start_time,
