...
def retry_failed_task(
    ...
    *,
    ...
    the failure on ``task_runs``. Each failed run can be explicitly requeued
    once. A literal ``failed`` task status without a run is accepted for
    compatibility with older/external boards; ordinary ready tasks are refused.
    """
    ...
    with transaction:
        ...
        already_retried = latest is not None and conn.execute(
            "SELECT 1 FROM task_events "
            "WHERE task_id = ? AND run_id = ? AND kind = 'retried' LIMIT 1",
            (task_id, int(latest["id"])),
        ).fetchone() is not None
        legacy_failed_without_run = task_row["status"] == "failed" and latest is None
        retryable_failed_run = (
            task_row["status"] in {"failed", "ready"}
            and failed_run
            and not already_retried
        )
        if not legacy_failed_without_run and not retryable_failed_run:
            return False
...
def check_respawn_guard(conn: sqlite3.Connection, task_id: str) -> Optional[str]:
    ...
    #    dragging done→ready, a dependency re-promotion, an unblock, a reclaim,
    #    or an explicit failed-run retry) is a deliberate "run it again" — honor
    #    it instead of deferring. Without this, a manual done→ready just sits there,
