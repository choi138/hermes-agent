...
def test_schema_exposes_only_user_owned_intake_fields():
    ...
    operation = intake.KANBAN_TASK_SCHEMA["parameters"]["properties"]["operation"]
...
def test_explicit_create_replays_legacy_omitted_operation_receipt(intake_env):
    tokens = _bind_discord_source(message_id="legacy-create-replay")
    try:
        implicit = _call({"title": "Backward-compatible create"})
        explicit = _call(
            {"operation": "create", "title": "Backward-compatible create"}
        )
    finally:
        clear_session_vars(tokens)

    assert explicit["task_id"] == implicit["task_id"]
...
def test_create_and_operation_receipts_share_one_conflict_namespace(intake_env):
    created = _create_from_message("Create owns this receipt", "shared-receipt")

    tokens = _bind_discord_source(message_id="shared-receipt")
    try:
        conflict = _call(
...
def test_status_replay_returns_committed_snapshot_across_session_restart(intake_env):
    created = _create_from_message("Stable status replay", "create-status-replay")

    tokens = _bind_discord_source(
        message_id="status-replay",
        session_id="session-before-restart",
    )
    try:
        first = _call({"operation": "status", "task_id": created["task_id"]})
    finally:
        clear_session_vars(tokens)

    with kb.connect_closing() as conn:
        assert kb.claim_task(conn, created["task_id"], claimer="status-replay-test")
    ...
    try:
        replay = _call({"operation": "status", "task_id": created["task_id"]})
    finally:
        clear_session_vars(tokens)
    ...
    assert replay["latest_run"] == first["latest_run"]
...
def test_retry_failed_run_can_only_be_requeued_once(intake_env):
    created = _create_from_message("Retry one failed run once", "create-one-retry")
    with kb.connect_closing() as conn:
        assert kb.claim_task(conn, created["task_id"], claimer="single-retry-test")
        assert not kb._record_spawn_failure(
            conn,
            created["task_id"],
            "worker failed to start",
    ...
    def retry(message_id):
        ...
        try:
            return _call({"operation": "retry", "task_id": created["task_id"]})
        finally:
            clear_session_vars(tokens)

    first = retry("first-explicit-retry")
    second = retry("second-explicit-retry")

    assert first["ok"] is True
    assert second["error"] == "kanban_task: cannot retry task in ready state"
    assert wake_calls == ["first-explicit-retry"]
    with kb.connect_closing() as conn:
        events = kb.list_events(conn, created["task_id"])
    assert sum(event.kind == "retried" for event in events) == 1
...
def test_retry_rolls_back_transition_when_receipt_write_fails(
    intake_env, monkeypatch
):
    created = _create_from_message("Atomic retry receipt", "create-atomic-retry")
    with kb.connect_closing() as conn:
        assert kb.block_task(conn, created["task_id"], reason="Still blocked")

    def fail_receipt(*_args, **_kwargs):
        raise RuntimeError("receipt write failed")

    monkeypatch.setattr(kb, "record_intake_operation", fail_receipt)
    wake_calls = []
    tokens = _bind_discord_source(
        message_id="atomic-retry",
        wake=lambda: wake_calls.append(True),
    )
    try:
        result = _call({"operation": "retry", "task_id": created["task_id"]})
    ...
    assert result["error"] == "kanban_task: receipt write failed"
    assert wake_calls == []
    with kb.connect_closing() as conn:
        task = kb.get_task(conn, created["task_id"])
        events = kb.list_events(conn, created["task_id"])
