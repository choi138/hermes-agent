...
def test_multiple_verification_retries_keep_candidates_internal(agent, monkeypatch):
    """Verification candidates stay internal until one becomes the final response."""
...
def test_configured_max_verify_nudges_bounds_stop_loop(agent, monkeypatch):
    """A configured one-nudge budget permits only one verification continuation."""
    agent.max_iterations = 3
    agent.iteration_budget.max_total = 3
    answers = iter([
        _response("candidate one"),
        _response("verified final report"),
        _response("unexpected third response"),
    ])
...
def test_verify_on_stop_does_not_emit_provisional_response_to_ui(agent, monkeypatch):
    """A completed-looking candidate is withheld while verification continues."""
    ...
    # At budget exhaustion the candidate is returned through the one final
    # response path, and the UI must not suppress it as already previewed.
    assert result["final_response"] == "composed report"
...
def test_streamed_verification_candidate_reused_marked_previewed(agent, monkeypatch):
    ...
    # The verification gate never emits a second completed-message callback.
    assert callback_calls == []
