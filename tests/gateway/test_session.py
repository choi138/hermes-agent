...
class TestRuntimeOverrides:
    def test_session_entry_runtime_override_roundtrip_is_secret_free(self):
        entry = SessionEntry(
            session_key="test",
            session_id="s1",
            created_at=datetime.now(),
        ...
        assert data["runtime_model"] == "gpt-5.5"
        assert data["runtime_provider"] == "codex-nekos"
        assert data["runtime_reasoning_effort"] == "high"
    ...
    def test_old_session_entry_defaults_runtime_fields_to_none(self):
    ...
    def test_session_store_runtime_override_update_and_clear(self, tmp_path):
        ...
        entry = store.get_or_create_session(
            SessionSource(platform=Platform.LOCAL, chat_id="cli", chat_type="dm")
        )
        ...
        assert store.clear_runtime_overrides(entry.session_key)
