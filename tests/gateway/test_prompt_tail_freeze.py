...
# block freeze (catalog-only static DesiredRoute + key-tuple cache +
# always-emitted reasoning_source), routed-turn composition stability, and the
# effective-delta eviction guard on gateway runtime overrides.
# ---------------------------------------------------------------------------


class TestRoutedTurnComposition:
    def test_routed_turn_does_not_move_the_composed_prompt(self, monkeypatch):
        ...
        routed = compose_effective_system_prompt(
            _agent({"label": "RUNTIME_OVERRIDE", "target_model": "gpt-5.6-sol"}),
            "BASE",
        )
...
class TestRuntimeRouteBlockFreeze:
    ...
    def test_reasoning_value_deterministic_for_effective_config(self):
        """The same effective config renders the same bytes whether the agent
        was rebuilt or reused after stale attribution is cleared."""
        from agent.system_prompt import build_runtime_route_block
...
def _override_runner(monkeypatch, *, current_model, switch_result, config_model=None):
    ...
    runner._persist_session_runtime_override = (
        lambda session_key, **kwargs: persisted.append((session_key, kwargs))
    )
    ...
    monkeypatch.setattr(
        "hermes_cli.model_switch.switch_model", lambda **kwargs: switch_result
    )
    monkeypatch.setattr(
...
class TestRuntimeOverrideEvictionGuard:
    def test_same_route_override_does_not_evict(self, monkeypatch):
        """Re-selecting the already-active route must not rebuild the agent."""
    ...
    def test_new_model_override_evicts_once(self, monkeypatch):
        ...
        assert "gpt-5.6-sol" in getattr(runner, "_pending_model_notes", {}).get(
            "sess-key", ""
        )

    def test_reasoning_only_override_never_evicts_and_persists(self, monkeypatch):
        """Reasoning is re-applied per turn and must survive gateway restart."""
        ...
        assert any(
            kwargs.get("include_reasoning")
            and kwargs.get("reasoning_config")
            == {"enabled": True, "effort": "max"}
            for _key, kwargs in persisted
        )


class TestCliComposition:
    def test_cli_composed_prompt_stable_across_calls(self):
        """Dormant route mode remains byte-stable and CurrentRuntime-only."""
        from agent.system_prompt import compose_effective_system_prompt
        ...
        assert "DesiredRoute:" not in first
