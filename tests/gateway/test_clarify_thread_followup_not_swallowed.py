...
arbitrary prose for native multi-choice prompts):
  ...
  * arbitrary prose is preserved for the next turn while the current clarify
    is released with an empty response so redirect-to-steer cannot deadlock
    behind the waiting tool; the user receives immediate choice guidance.
...
async def test_thread_prose_not_swallowed_by_native_multi_choice_clarify():
    ...
    # The prose is not accepted as the answer. It is queued for the next turn
    # and the clarify is released with an empty response so the blocked tool
    # can finish and drain that queued message.
    with cm._lock:
