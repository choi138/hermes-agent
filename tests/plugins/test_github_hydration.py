...
def _files_client(payload: Any, requests: list[Any] | None = None):
    def transport(request: Any, timeout: float) -> GitHubHttpResponse:
    ...
    return GitHubNotificationsClient(token="test-token", transport=transport)


def test_client_reads_pull_files_with_get_only_and_bounds_the_patch() -> None:
    requests: list[Any] = []
    client = _files_client(
        [
            {
                "filename": "Toki/Features/UsagePanel/PanelTabBarView.swift",
                "status": "modified",
                "patch": "@@ -1,2 +1,2 @@\n-old()\n+new()",
            },
            {
    ...
    result = client.fetch_pull_files(SUBJECT_URL, repository=REPOSITORY, limit=30)
    ...
    # A binary file simply has no diff; that is not a failure.
    assert result.files[1].patch == ""
    assert result.files[1].patch_truncated is False
    assert result.files[2].patch_truncated is True
    assert len(result.files[2].patch) <= 20_000

    parsed = urlparse(requests[0].full_url)
    assert parsed.path == "/repos/silviahealth/content/pulls/7/files"
    assert parse_qs(parsed.query) == {"per_page": ["30"]}
    assert requests[0].get_method() == "GET"
    assert requests[0].data is None


def test_client_reports_a_full_page_of_pull_files_as_truncated() -> None:
    client = _files_client(
        [{"filename": f"file-{index}.py", "status": "modified"} for index in range(4)]
    )

    result = client.fetch_pull_files(SUBJECT_URL, repository=REPOSITORY, limit=4)

    assert result.truncated is True
    assert result.unavailable is False


def test_oversized_pull_files_response_degrades_instead_of_retrying_forever() -> None:
    """A protocol error here is retryable, which would livelock the poll loop.

    _decode_json rejects a body over _MAX_RESPONSE_BYTES with a retryable
    protocol_error, and runtime backoff would keep retrying it and never deliver
    the notification. Wider diff context is optional, so the read degrades.
    """

    client = _files_client(b"x" * (1_048_576 + 1))

    result = client.fetch_pull_files(SUBJECT_URL, repository=REPOSITORY)
...
def test_pull_files_read_is_refused_for_a_non_pull_subject() -> None:
    client = _files_client([{"filename": "a.py", "status": "modified"}])

    result = client.fetch_pull_files(
        f"https://api.github.com/repos/{REPOSITORY}/issues/7",
...
@pytest.mark.parametrize("limit", [0, 101, True])
def test_client_rejects_unbounded_pull_files_limit(limit: Any) -> None:
    client = _files_client([])

    with pytest.raises(ValueError, match="limit"):
        client.fetch_pull_files(SUBJECT_URL, repository=REPOSITORY, limit=limit)
