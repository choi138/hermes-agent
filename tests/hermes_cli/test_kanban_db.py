...
def test_detect_crashed_workers_reclaims_reused_pid_after_restart(
    ...
    observed_start = {77777: 100}
    monkeypatch.setattr(
        _kb,
        "get_process_start_time",
        lambda pid: observed_start.get(int(pid)),
        raising=False,
    )
    ...
    with kb.connect() as conn:
        ...
        stored = conn.execute(
            "SELECT worker_start_time FROM task_runs WHERE id = ?", (run_id,),
        ).fetchone()
        assert stored["worker_start_time"] == 100
...
def test_classify_worker_exit_is_scoped_to_worker_start_time(
    kanban_home, monkeypatch,
):
    import hermes_cli.kanban_db as _kb
    ...
    pid = 77888
    monkeypatch.setattr(
        _kb, "_owned_worker_start_times", {pid: 100}, raising=False,
    )
    _kb._record_worker_exit(pid, _exited_status(1))

    assert _kb._classify_worker_exit(pid, 100) == ("nonzero_exit", 1)
    assert _kb._classify_worker_exit(pid, 200) == ("unknown", None)
...
def test_terminate_owned_descendants_does_not_signal_reused_pid(monkeypatch):
    ...
    monkeypatch.setattr(kb, "_owned_worker_start_times", {pid: 100})
    monkeypatch.setattr(kb, "get_process_start_time", lambda _pid: 200)
