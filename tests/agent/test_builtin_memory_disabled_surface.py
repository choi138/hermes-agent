...
class TestIndependentStoreWriteGates:
    ...
    def test_user_profile_only_rejects_memory_write(self, hermes_home):
        ...
        # Fork contract: add/replace require a suitability reason
        # (_validate_memory_write_reason); the upstream original omitted it.
        _reason = "Durable environment fact needed across future sessions."
        store = self._store(memory_enabled=False, user_profile_enabled=True)
        denied = json.loads(memory_tool(action="add", target="memory", content="fact", reason=_reason, store=store))
        allowed = json.loads(memory_tool(action="add", target="user", content="pref", reason=_reason, store=store))
    ...
    def test_memory_only_rejects_user_profile_write(self, hermes_home):
        ...
        denied = json.loads(memory_tool(action="add", target="user", content="pref", reason=_reason, store=store))
        allowed = json.loads(memory_tool(action="add", target="memory", content="fact", reason=_reason, store=store))
