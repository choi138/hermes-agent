...
def test_aggregate_summary_input_is_bounded_and_keeps_both_edges():
    """Lean mode (default): oversized input is EVEN-SAMPLED across the whole
    region with elision markers, still bounded, still covering both edges."""
    compressor = _compressor()

    with patch(
        "agent.context_compressor.call_llm", return_value=_response("bounded")
    ) as call:
        summary = compressor._generate_summary(_oversized_messages())
...
def test_legacy_mode_keeps_head_tail_truncation_marker():
    """Legacy tail mode keeps the original head+tail bound and its marker."""
    compressor = _compressor()
