...
def classify_responses_route(agent: Any) -> ResponsesRouteFlags:
    ...
    # Custom providers can front the codex backend (``custom:<name>`` runtime
    # ids, declared backend families) — normalize both the live and the
    # requested provider id before comparing, and honor the declared backend
    # family, so a codex-lb style proxy still classifies as codex (fork).
    ...
    runtime_ids = {
        ...
        for value in (provider_lower, requested_provider)
        if value
    }
    backend_family = None
    try:
    ...
    is_codex_backend = (
        ...
        or (_host_is("chatgpt.com") and "/backend-api/codex" in lower)
    )
