...
def build_cache_parity_fork(
    ...
    *,
    ...
    review_agent = AIAgent(
        ...
        # primary-provider outage exactly like the main conversation does.
        # Without this the fork's chain is [] (see agent_init), so
        # _try_activate_fallback() can never succeed and a 429/503 on the
        # primary kills the review with calls=0. ``routed`` also covers a
        # model-only change. Keep the chain only when the fork resolves to
        # the parent's primary provider family, endpoint, and credential
        # route. An empty parent chain normalizes to None in every branch.
        fallback_model=(
    ...
    # memory manager, but the ingest-curator recipe (and any future variant)
    # rebinds the PARENT's manager onto the fork for memory_search read
    # access — this flag is what keeps that safe: every sync/ingest/prefetch
    # call site gates on it (agent.memory_manager.memory_ingest_allowed),
    # so the fork's harness prompt can never reach the external graph.
...
def _run_review_in_thread(
    ...
    if review_run is not None and review_run.cancel_requested.is_set():
        ...
        # Report failure so the coordinator does not record this key as
        # completed: a startup-cancelled review never ran, and marking it
        # done would dedupe the retry away on the next turn.
        return False
        ...
        with thread_scoped_silence():
            ...
            try:
                ...
                if request_admitted:
                    ...
                    # Only an admitted (actually executed) review counts as
                    # success for the coordinator's dedupe ledger. A review
                    # fenced out by cancellation must stay retryable.
