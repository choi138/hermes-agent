...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # contract (fork). Keep a standalone block only when no index is
    # available at this point; the docs-only variant covers sessions where
    # skill_view() would be a dangling reference. The slot records where the
    # block sits so the post-index back-patch below (upstream v2026.8.31)
    # can upgrade it to the skill-pointer variant if the index materializes
    # later with the hermes-agent skill installed. Toolset and skill set are
    # fixed per-session, so cache-safe either way.
    _has_skill_view = "skill_view" in (agent.valid_tool_names or set())
    _help_guidance_slot: Optional[int] = None
