...
class McpCredentialRedirectError(RuntimeError):
    """Raised when an MCP redirect would cross origins with credentials."""
...
_MCP_AMBIENT_CREDENTIAL_HEADERS = frozenset({
    "authorization",
    "proxy-authorization",
    "cookie",
})
_MCP_NON_CREDENTIAL_REDIRECT_HEADERS = frozenset({"mcp-protocol-version"})


def _configured_mcp_credential_header_names(headers: Any) -> set[str]:
    if not isinstance(headers, dict):
        return set()
    return {
        str(name).lower()
        for name in headers
        if str(name).lower() not in _MCP_NON_CREDENTIAL_REDIRECT_HEADERS
    }


def _enforce_mcp_cross_origin_redirect_boundary(
    response: Any,
    *,
    original_url: Any,
    configured_header_names: set[str],
    client_cert: Any,
    auth_configured: bool = False,
) -> None:
    """Stop a credential-bearing cross-origin redirect before its next hop.

    Mutating ``response.next_request`` from a response hook does not change the
    request httpx later constructs for the real redirect flow.  Inspect the
    response ``Location`` instead and raise from the hook, which runs before
    httpx sends the redirected request.  Client certificates are transport-
    scoped, and custom/auth headers may be regenerated by auth handlers, so a
    credential-bearing cross-origin redirect must fail closed rather than be
    rewritten in place.
    """
    ...
    try:
        ...
        target = request.url.join(location) if location else None
    ...
    ambient_credentials_present = any(
        header_name in request_headers
        for header_name in _MCP_AMBIENT_CREDENTIAL_HEADERS
    )
    if client_cert is not None:
        raise McpCredentialRedirectError(
            "MCP cross-origin redirect blocked because a client certificate is configured"
        )
    if configured_header_names or auth_configured or ambient_credentials_present:
        raise McpCredentialRedirectError(
            "MCP cross-origin redirect blocked because credential-bearing headers or auth are configured"
        )
...
class MCPServerTask:
    ...
    async def _preflight_content_type(
        ...
        *,
        ...
        strict_loopback = (
            follow_redirects is False and _strict_loopback_mcp_url_is_safe(url)
        )
        probe_headers = dict(headers) if headers else {}
        original_url = _httpx.URL(url)
        configured_header_names = _configured_mcp_credential_header_names(
            probe_headers
        )

        async def _enforce_preflight_redirect_boundary(response):
            if follow_redirects:
                _enforce_mcp_cross_origin_redirect_boundary(
                    response,
        ...
        client_kwargs: dict = {
            ...
            "trust_env": not strict_loopback,
            "timeout": _httpx.Timeout(timeout),
            "event_hooks": {"response": [_enforce_preflight_redirect_boundary]},
        }
        if client_cert is not None:
    ...
    async def _run_http(self, config: dict):
        ...
        _ensure_mcp_sdk()
        is_sse = config.get("transport") == "sse"
        if not is_sse and not _MCP_HTTP_AVAILABLE:
            raise ImportError(
        ...
        configured_header_names = _configured_mcp_credential_header_names(headers)
        ...
        if client_cert is not None and not is_sse and not _MCP_NEW_HTTP:
            raise RuntimeError(
                "MCP legacy HTTP transport cannot enforce configured client certificates"
            )
        ...
        if is_sse:
            ...
            if requires_sse_factory and not _callable_accepts_keyword(
                ...
                raise RuntimeError(
                    f"MCP server '{self.name}' requires SSE client security "
                    "controls unsupported by the installed MCP SDK"
                )
            if requires_sse_factory:
                ...
                # Use the SDK's httpx generation (httpx2 on mcp >= 2.0).
                ...
                _original_sse_url = _httpx_mod.URL(url)
                _configured_sse_header_names = configured_header_names

                async def _enforce_sse_cross_origin_redirect_boundary(response):
                    if _follow_redirects_for_factory:
                        _enforce_mcp_cross_origin_redirect_boundary(
                            response,
                            original_url=_original_sse_url,
                            configured_header_names=_configured_sse_header_names,
                            client_cert=_cert_for_factory,
                            auth_configured=_oauth_auth is not None,
                        )
                ...
                def _mcp_http_client_factory(
                    ...
                    kwargs: dict = {
                        ...
                        "event_hooks": {
                            "response": [_enforce_sse_cross_origin_redirect_boundary]
                        },
                    }
        ...
        if _MCP_NEW_HTTP:
            ...
            _configured_header_names = configured_header_names

            async def _enforce_cross_origin_redirect_boundary(response):
                """Stop credential-bearing cross-origin redirects before the next hop."""
                if follow_redirects:
                    _enforce_mcp_cross_origin_redirect_boundary(
                        response,
            ...
            client_kwargs: dict = {
                ...
                "verify": ssl_verify,
                "event_hooks": {"response": [_enforce_cross_origin_redirect_boundary]},
            }
            if headers:
            ...
            if configured_header_names or _oauth_auth is not None:
                raise RuntimeError(
                    f"MCP server '{self.name}' has credentials configured, but "
                    "the installed MCP SDK cannot enforce the cross-origin redirect boundary"
                )
            _http_kwargs: dict = {
                "headers": headers,
                "timeout": float(connect_timeout),
    ...
    async def run(self, config: dict):
        ...
        if self._is_http():
            ...
            if (
                ...
                and not _MCP_NEW_HTTP
                and (
                    config.get("follow_redirects") is False
                    or config.get("client_cert") is not None
                    or bool(
                        _configured_mcp_credential_header_names(
                            config.get("headers")
                        )
                    )
                    or self._auth_type == "oauth"
                )
            ):
                self._error = RuntimeError(
                    "MCP legacy HTTP transport cannot enforce strict redirect, credential, or client-certificate policy"
                )
                self._ready.set()
                return
            ...
            if config.get("transport") != "sse" and not config.get("skip_preflight") and not self._ready.is_set() and self._auth_type != "oauth":
                ...
                except (NonMcpEndpointError, McpCredentialRedirectError) as exc:
                    logger.warning("%s", exc)
                    self._error = exc
                    self._ready.set()
                    return
    ...
    def _validate_live_server_binding(self) -> Any:
        with _lock:
            ...
            if (
                ...
                or _read_only_registry_attestation(registered_names)
...
def bind_read_only_mcp_tool(
    *,
    ...
    with _lock:
        ...
        registry_attestation = _read_only_registry_attestation(registered_tool_names)
